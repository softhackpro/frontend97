  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log(result );
        setUserData((prev) => ({ ...prev, name:result.data.profile_info.user_id,commission:result.data.profile_info.match_commission , exposureLimit:result.data.profile_info.min_sports_exp }))
      } catch (error) {
        console.log(error);
      }
    }
  ...
  return (
    <>
      <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg overflow-hidden border border-gray-300">
        <div className="bg-gray-800 text-white text-lg font-semibold p-3">Account Details</div>
        <div className="p-4">
          ...
          <div className="mb-3 border-b pb-2">
            ...
            <AiFillEye className="text-xl text-gray-700" onClick={() => setIsOpen(true)} />
          </div>
          <div className="mb-3 border-b pb-2">
          ...
          <div className="mb-3 border-b pb-2 flex items-center gap-2">
            ...
            <button className="flex items-center">
              Edit <FiEdit className="ml-1" onClick={() => setModalOpen(true)} />
            </button>
          </div>
        </div>
      </div>
